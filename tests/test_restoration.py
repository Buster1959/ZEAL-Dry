...
from custom_components.zeal_dry.state_machine import StateSnapshot, ControllerState, TimingConfig, next_state
...
def test_startup_never_bypasses_rest():
    for data in (None, {}, {'state':'drying'}, {'drying_started_at':'bad'}):
        ...
        assert next_state(snapshot,DEMAND,NOW,TimingConfig(),action_permitted=True).state != ControllerState.DRYING
        assert next_state(snapshot,DEMAND,NOW+timedelta(minutes=10),TimingConfig(),action_permitted=True).state == ControllerState.DRYING
...
def test_fault_and_inhibit_record_stop():
    ...
    assert next_state(fault,DEMAND,NOW,TimingConfig(),action_permitted=True).state != ControllerState.DRYING
    inhibited = next_state(running,DEMAND,NOW,TimingConfig(),action_permitted=True,inhibited=True)
