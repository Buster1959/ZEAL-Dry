...
class ClimateAdapter:
    ...
    def target(self, target_c, minimum_c, maximum_c):
        ...
        attrs = self.inspect()
        if not int(attrs.get("supported_features", 0)) & ClimateEntityFeature.TARGET_TEMPERATURE:
            return None
        unit = attrs.get("temperature_unit", self.hass.config.units.temperature_unit)
        convert = lambda value: TemperatureConverter.convert(value, UnitOfTemperature.CELSIUS, unit)
        try:
    ...
    async def _call(self, service, **data):
        await self.hass.services.async_call("climate", service, {"entity_id": self.entity_id, **data}, blocking=True)
    ...
    async def async_dry(self, target_c, minimum_c, maximum_c):
        ...
        self.last_result = "dry_requested" if target is not None else "dry_requested_without_temperature_support"
